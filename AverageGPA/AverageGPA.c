...
int Input(float (*GPA_point)[2], int *num_class) {
    ...
    printf("Waiting for reading files from GPA.csv...\n");
    FILE *fp;
    if ((fp = fopen("GPA.csv", "r")) == NULL) {
        printf("The GPA.csv file does not exist! Please put \
        GPA.csv under the same path of the execution file.");
    }
    int i = 0;
    ...
    while (fgets(buffer, MAX_SIZE, fp) != NULL) {
        ...
        while (result != NULL) {
            ...
            j++;
            if (result == NULL && j != 7) {
                printf("Please check your csv file! There some blanks have "
                       "not been filled yet.\n");
                return -1;
            }
        }
...
int Calculate(float (*GPA_point)[2], int num_class, float *averScore,
              float *averGPA, float *total_Credit) {
    *total_Credit = 0;
